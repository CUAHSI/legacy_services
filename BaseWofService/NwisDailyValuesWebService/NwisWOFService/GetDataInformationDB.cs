...
namespace NwisWOFService
{
    namespace v1_0 
    {
        ...
        using DataInfoService = WaterOneFlow.Service.v1_0.DataInfoService;
    ...
    public class GetDataInformationDB : DataInfoService
    {
        ...
        private bool useVariableCodeMatch = true;

        public bool UseVariableCodeMatch
        {
            get { return useVariableCodeMatch; }
            set { useVariableCodeMatch = value; }
        }
        private bool useSitesCodeMatch = true;

        public bool UseSitesCodeMatch
        {
            get { return useVariableCodeMatch; }
            set { useVariableCodeMatch = value; }
        }
            ...
            else
            {
                foreach (VariableParam v in variables)
                {
                    ...
                    try
                    {
                        ...
                        {
                            string variableQuery = string.Format(Properties.Settings.Default.VariableCodeQueryFormat,
                                                                 VariablesTableName);

                            SqlCommand command = new SqlCommand(variableQuery, conn);
                            if (useVariableCodeMatch)
                            {
                                SqlParameter vCodeParam = new SqlParameter("variableCode", v.OrignalCodeNoVocabulary);
                        ...
                        where = VariableOptions2WhereClause(v);
                       }     /* Use Caution, where clause not SQL inject safe. */
            ...
            else
            {
                if (lp[0].isGeometry)
                {
                    using (SqlConnection conn = new SqlConnection(DataInfoConnection))
                    {
                        ...
                        string query =
                            ODMSqlQueries.GeomToSqlWhere(lp[0].Geometry, SitesTableName);
                ...
                else
                {
                    foreach (locationParam loc in lp)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(DataInfoConnection))
                        {
                            // same format string for both sites and series. Table Name changes
                            string query =
                                string.Format(Properties.Settings.Default.SiteCodeQueryFormat, SitesTableName);

                            SqlCommand comm = new SqlCommand(query, conn);

                            SqlParameter param = new SqlParameter("siteCode", siteCodeWithAgency(loc));

                            comm.Parameters.Add(param);
                            SqlParameter param2 = null;
                            if (loc.options.ContainsKey("agency"))
                            {
                              param2 = new SqlParameter("agency",loc.options["agency"].Trim());
                            }
                            else
                            {
                                param2 = new SqlParameter("agency", "USGS");
                            }
 
                                // need to check location for agency
        ...
        private static SiteInfoType row2SiteInfoType(DataRow row)
        {
            ...
            // add agency note
            if (columns.Contains("usgs_agency") && row["usgs_agency"] != DBNull.Value)
            {
            ...
            // add siteType note
            if (columns.Contains("usgs_station_type") && row["usgs_station_type"] != DBNull.Value)
            {
                NoteType n = new NoteType();
                n.title = "SiteType";
        ...
        public static  string siteCodeWithAgency(locationParam locationParam)
        {
            if(locationParam.options.ContainsKey("agency"))
            {
                if (locationParam.options["agency"].Trim().Equals("USGS",StringComparison.InvariantCultureIgnoreCase))
                {
                    return locationParam.SiteCode;
                } else
                {
                    return String.Format("{0}/agency={1}", locationParam.SiteCode, locationParam.options["agency"]);
                }
        ...
        public override seriesCatalogType[] GetSeries(locationParam lp, TimeSeriesTypeEnum seriesType)
        {
           if(Parameters.ContainsKey("SeriesType"))
           {
               seriesType = (TimeSeriesTypeEnum)Parameters["SeriesType"];
           }
            ...
            #region build series
            {
                ...
                if (lp != null)
                {
                    if (lp.SiteCode != null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(DataInfoConnection))
                        {
                            ...
                            SqlCommand comm = new SqlCommand(query, conn);
                            SqlParameter param = new SqlParameter("siteCode", siteCodeWithAgency(lp));
                            comm.Parameters.Add(param);
                            SqlParameter param2 = null;
                            if (lp.options.ContainsKey("agency"))
                            {
                                param2 = new SqlParameter("agency", lp.options["agency"].Trim());
                            }
                            else
                            {
                                param2 = new SqlParameter("agency", "USGS");
                ...
                {
                    seriesCatalogTypeSeries s = row2Series(row, seriesType);
        ...
        {
            return row2Series(aRow, TimeSeriesTypeEnum.Interval);
        }

        private static seriesCatalogTypeSeries row2Series(DataRow aRow, TimeSeriesTypeEnum seriesType)
        {
